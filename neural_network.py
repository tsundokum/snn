...
def gradient_check(e, m, X, Y, input_relation, theta_1, theta_2, theta_relation,
    ...
    for i in xrange(num_lay_1):  # loop over the weight matrices in the first subnetwork
        ...
        for p in xrange(np.size(theta_1[i])):  # loop over all of the elements in the current weight matrix
            ...
            [a_chm_1, a_chm_2] = forward_propagation(m, num_lay_1, num_lay_2, X,
                    input_relation, th_ch_minus, theta_2, theta_relation)      # perform forward propagation with new weight
            j_ch_minus = compute_cost_function(m, a_chm_2, th_ch_minus, theta_2,
                    theta_relation, num_lay_1, num_lay_2, R, Y)     # compute  new cost function
            # Compute "plus" error:
            [a_chp_1, a_chp_2] = forward_propagation(m, num_lay_1, num_lay_2, X,
                    input_relation, th_ch_plus, theta_2, theta_relation)  # perform forward propagation with new weight
    ...
    for p in xrange(np.size(theta_relation)):   # loop over all of the elements in the current weight matrix
        ...
        # Compute "minus" error:
        [a_chm_1, a_chm_2] = forward_propagation(m, num_lay_1, num_lay_2, X,
                input_relation, theta_1, theta_2, th_ch_minus)          # perform forward propagation with new weight
        j_ch_minus = compute_cost_function(m, a_chm_2, theta_1, theta_2,
                th_ch_minus, num_lay_1, num_lay_2, R, Y)                # compute  new cost function
        # Compute "plus" error:
        [a_chp_1, a_chp_2] = forward_propagation(m, num_lay_1, num_lay_2, X,
                input_relation, theta_1, theta_2, th_ch_plus)       # perform forward propagation with new weight
        j_ch_plus = compute_cost_function(m, a_chp_2, theta_1, theta_2,
                th_ch_plus, num_lay_1, num_lay_2, R, Y)             # compute  new cost function
    ...
    for i in xrange(num_lay_2 + 1):  # loop over the weight matrices in the second subnetwork
        ...
        for p in xrange(np.size(theta_2[i])):  # loop over all of the elements in the current weight matrix
            ...
            # Compute "minus" error:
            [a_chm_1, a_chm_2] = forward_propagation(m, num_lay_1, num_lay_2, X,
                    input_relation, theta_1, th_ch_minus, theta_relation)   # perform forward propagation with new weight
            ...
            # Compute "plus" error:
            [a_chp_1, a_chp_2] = forward_propagation(m, num_lay_1, num_lay_2, X,
                    input_relation, theta_1, th_ch_plus, theta_relation)   # perform forward propagation with new weight
